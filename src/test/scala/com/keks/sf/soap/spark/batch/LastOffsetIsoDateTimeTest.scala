...
class LastOffsetIsoDateTimeTest extends TestBase with MockedServer with DataFrameEquality {
  ...
  def tests(sfFormat: String): Unit = {
    it should "load data with IsoDateTime offset" in withMockedServer(wireMockServerHost, wireMockServerPort) { _ =>
      ...
      stubSeqOfScenarios("test_scenario")(
        ...
        post(requestUrl)
          .withRequestBody(containing(
            qs"""SELECT Id,TimeField,SystemModstamp FROM User
                |WHERE (isDeleted = false) AND
                |(SystemModstamp >= 2020-01-01T00:00:02.000Z AND SystemModstamp &lt;= 2020-01-02T00:00:00.000Z)
                |ORDER BY SystemModstamp""".stripMargin))
