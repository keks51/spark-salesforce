...
class StreamingRecoveryAndPollNewTest extends TestBase with MockedServer with DataFrameEquality with TmpDirectory {
  ...
  "Spark with streaming v2 api" should "load all available data and poll new" in withMockedServer(wireMockServerHost, wireMockServerPort) { _ =>
    withTempDir { dir =>
      ...
      createFile(checkPointDir,
                 ...
                 """{"partitions":[],"maxUpperBoundOffset":"2020-04-02T00:00:00.000Z"}""")
      createFile(sfExecutorsPartition0Dir,
                 SF_EXECUTOR_PARTITION_FILE_NAME,
                 """{"id":0,"column":"SystemModstamp","lowerBound":"2020-02-02T00:00:00.000Z","upperBound":"2020-04-02T00:00:00.000Z","leftCondOperator":">=","rightCondOperator":"<=","executorMetrics":{"offset":"2020-04-02T00:00:00.000Z","isDone":true,"queryLocator":null,"loadedRecords":4,"recordsToLoad":4,"batchResponseTimeAvg":7,"batchSfProcessingTimeAvg":3,"partitionProcessingTime":11,"recordsInSfBatch":2}}""")
      ...
      verify(WireMock.exactly(12), postRequestedFor(requestUrl)) // +1 duet to last bad request
