...
class StreamingBySystemmodstampTest extends TestBase with MockedServer with TmpDirectory with DataFrameEquality {
  ...
  "Spark with streaming v2 api" should "load all available data by SystemModstamp" in withMockedServer(wireMockServerHost, wireMockServerPort) { _ =>
    withTempDir { dir =>
      ...
      val firstOffset1 = Seq(
        Seq(SfRecord(SYSTEMMODSTAMP, Some("2000-01-01T00:00:00.000Z")))
      )
