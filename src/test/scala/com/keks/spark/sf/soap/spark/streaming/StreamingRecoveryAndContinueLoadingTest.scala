...
class StreamingRecoveryAndContinueLoadingTest extends TestBase with MockedServer with DataFrameEquality with TmpDirectory {
  ...
  "Spark with streaming v2 api" should "load all available data and poll new" in withMockedServer(wireMockServerHost, wireMockServerPort) { _ =>
    withTempDir { dir =>
      val checkPointDir = s"$dir/$CHECKPOINT_DIR"
      val commitsDir = s"$checkPointDir/commits"
      val offsetsDir = s"$checkPointDir/offsets"
      val sfExecutorsDir = s"$checkPointDir/$SF_EXECUTORS_PARTITIONS_DIR"
      val sfExecutorsPartition0Dir = s"$sfExecutorsDir/0"
      createDir(checkPointDir)
      createDir(commitsDir)
      createDir(offsetsDir)
      createDir(sfExecutorsDir)
      createDir(sfExecutorsPartition0Dir)
      createFile(commitsDir,
                 "0",
                 """v1
                   |{"nextBatchWatermarkMs":0}""".stripMargin)
      createFile(commitsDir,
                 "1",
                 """v1
                   |{"nextBatchWatermarkMs":0}""".stripMargin)
      createFile(offsetsDir,
                 "0",
                 """v1
                   |{"batchWatermarkMs":0,"batchTimestampMs":1611821189936,"conf":{"spark.sql.streaming.stateStore.providerClass":"org.apache.spark.sql.execution.streaming.state.HDFSBackedStateStoreProvider","spark.sql.streaming.flatMapGroupsWithState.stateFormatVersion":"2","spark.sql.streaming.multipleWatermarkPolicy":"min","spark.sql.streaming.aggregation.stateFormatVersion":"2","spark.sql.shuffle.partitions":"20"}}
                   |2000-01-01T00:00:00.000Z""".stripMargin)
      createFile(offsetsDir,
                 "1",
                 """v1
                   |{"batchWatermarkMs":0,"batchTimestampMs":1611821192911,"conf":{"spark.sql.streaming.stateStore.providerClass":"org.apache.spark.sql.execution.streaming.state.HDFSBackedStateStoreProvider","spark.sql.streaming.flatMapGroupsWithState.stateFormatVersion":"2","spark.sql.streaming.multipleWatermarkPolicy":"min","spark.sql.streaming.aggregation.stateFormatVersion":"2","spark.sql.shuffle.partitions":"20"}}
                   |2020-02-02T00:00:00.000Z""".stripMargin)
      createFile(checkPointDir,
                 "metadata",
                 """{"id":"2dda91ee-5895-44a6-9e89-a6731de7b5a9"}""")
      createFile(checkPointDir,
                 SF_DRIVER_PARTITIONS,
                 """{"partitions":[{
                   |  "id": 0,
                   |  "column": "SystemModstamp",
                   |  "lowerBound": "2020-02-01T00:00:00.000Z",
                   |  "upperBound": "2020-04-02T00:00:00.000Z",
                   |  "leftCondOperator": ">=",
                   |  "rightCondOperator": "<=",
                   |  "offsetValueIsString": false,
                   |  "executorMetrics": {
                   |    "offset": "2020-02-01T00:00:00.000Z",
                   |    "isDone": false,
                   |    "queryLocator": null
                   |  }
                   |}],"maxUpperBoundOffset":"2020-04-02T00:00:00.000Z"}""".stripMargin)
      createFile(sfExecutorsPartition0Dir,
                 SF_EXECUTOR_PARTITION_FILE_NAME,
                 """{"id":0,"column":"SystemModstamp","lowerBound":"2020-01-01T00:00:00.000Z","upperBound":"2020-02-02T00:00:00.000Z","leftCondOperator":">=","rightCondOperator":"<=","executorMetrics":{"offset":"2020-04-02T00:00:00.000Z","isDone":true,"queryLocator":null,"loadedRecords":4,"recordsToLoad":4,"batchResponseTimeAvg":7,"batchSfProcessingTimeAvg":3,"partitionProcessingTime":11,"recordsInSfBatch":2}}""")
      val saveDir = s"$dir/result"
      val spark_metadata_dir = s"$dir/result/_spark_metadata"
      createDir(saveDir)
      createDir(spark_metadata_dir)
      val file1 = s"$saveDir/part1.json"
      val file2 = s"$saveDir/part2.json"
      createFile(saveDir, "part1.json", """{"Id":"a","TimeField":"00:00:00.000Z","SystemModstamp":"2020-01-01T00:00:00.000+03:00"}""")
      createFile(saveDir, "part2.json", """{"Id":"b","TimeField":"00:00:00.000Z","SystemModstamp":"2020-01-02T00:00:00.000+03:00"}""")
      val file1Path = new Path(s"file:///$file1")
      val file2Path = new Path(s"file:///$file2")
      createFile(spark_metadata_dir,
                 "0",
                 s"""v1
                    |{"path":"$file1Path","size":942,"isDir":false,"modificationTime":1611832481811,"blockReplication":1,"blockSize":33554432,"action":"add"}""".stripMargin)
      createFile(spark_metadata_dir,
                 "1",
                 s"""v1
                    |{"path":"$file2Path","size":942,"isDir":false,"modificationTime":1611832482658,"blockReplication":1,"blockSize":33554432,"action":"add"}""".stripMargin)
      ...
      val sfQueryResultXmlResponse7 = SfQueryResultXmlResponse("User", sfRecordsList7, Some("locator_4")).toString
      ...
      stubSeqOfScenarios("test_scenario")(
        ...
        post(bindUrl).willReturn(aResponse().withBody(bindingXmlResponse)),
        post(requestUrl)
          .withRequestBody(containing(s"<m:queryString>$selectQuery WHERE (isDeleted = false) AND ($SYSTEMMODSTAMP >= 2020-02-01T00:00:00.000Z AND $SYSTEMMODSTAMP &lt;= 2020-04-02T00:00:00.000Z) $orderBySoql</m:queryString>"))
      ...
      val streamDF =
        spark
          ...
          .option(SF_STREAMING_QUERY_NAME, "User recovery")
      ...
      val result = spark
        .read
        .schema(StructType(Array(StructField(ID, StringType), StructField(TIME_FIELD, StringType), StructField(SYSTEMMODSTAMP, TimestampType))))
        .json(saveDir)
      result.show(false)
      val expected = Seq(
      ...
      verify(WireMock.exactly(3), postRequestedFor(bindUrl))
      verify(WireMock.exactly(8), postRequestedFor(requestUrl)) // +1 duet to last bad request
